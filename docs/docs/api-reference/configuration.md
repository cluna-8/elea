...
### Sentinel Guardian - Environment Variables
...
| `REDIS_HOST` | — | Host de Redis. OBLIGATORIO en producción: el compose de prod lo pasa con `:?`, sin default. Lo usan DOS servicios (backend y motor del gateway) — los dos escriben y leen las mismas claves (`sentinel:audit:*`, `sentinel:nlp:*`); apuntar a instancias distintas hace que el health cuente cero mientras el otro plano registra. En dev lo levanta el compose; en prod es el host de la instancia gestionada por el cliente. |
...
Un pico de generaciones lentas no puede llevarse puesto el login, el health ni el panel. El backend admite hasta N pedidos EN VUELO hacia el motor y al resto lo rechaza rápido —503 honesto, auditado y con cabecera `X-Sentinel-Rejected: saturated`— en vez de encolarlo reteniendo una conexión de Postgres mientras espera. Todas las perillas de este bloque son POR PROCESO: el total de la instalación es el valor × WEB_CONCURRENCY (los workers no comparten memoria, así que tampoco comparten el semáforo). Un valor ausente, vacío, malformado o FUERA DE RANGO nunca tumba el arranque: se loguea un warning y se usa el default. Fuente de los rangos: backend/src/services/engine_gate.py.

| Variable | Default | Descripción |
|---|---|---|
| `SENTINEL_ENGINE_MAX_CONCURRENCY` | `8` | Pedidos en vuelo hacia el motor por proceso — la perilla del TOPE. Rango válido: 1..64 (fuera de ahí se ignora con warning). Cada pedido admitido retiene una conexión del pool (30 por proceso): con 8 quedan ~22 libres para login, admin y health. Un tope demasiado alto no es "más capacidad", es volver a no tener tope. ⚠ SI SUBÍS ESTA PERILLA, SUBÍ TAMBIÉN el `max_parallel_requests` del deployment local en el catálogo del motor. Invariante: max_parallel_requests > SENTINEL_ENGINE_MAX_CONCURRENCY × WEB_CONCURRENCY (hoy 20 > 8 × 2 = 16). Si se invierte, el primero en rechazar pasa a ser el motor —error opaco, sin cabecera `X-Sentinel-Rejected` y sin fila auditada— en vez del backend, y saturar deja de verse. Nada lo verifica en runtime: es tuya al girar el tope. |
| `SENTINEL_ENGINE_QUEUE_TIMEOUT_SECONDS` | `5` | Cuánto espera su turno un pedido antes de que se lo rechace, en segundos — la perilla de la PACIENCIA. Rango válido: 0 < v <= 60 (un queue-timeout de 600 s es la cola infinita con otro nombre). El `Retry-After` del 503 se deriva de este valor, no es un 5 fijo. |
| `SENTINEL_ENGINE_TIMEOUT_SECONDS` | `60` | Timeout de la llamada al motor desde el CHAT de la consola, en segundos. Rango válido: 0 < v <= 600. Es una UI con una persona esperando, por eso el default es corto; el perfil de producción lo sube a 150 para no cortar antes que el router del motor (120 s con modelos locales lentos), que sería abortar una respuesta buena y ya pagada. |
| `SENTINEL_GW_BYOK_TIMEOUT_SECONDS` | `150` | Timeout TOTAL de una llamada BYOK no-streaming de /gw (herramientas de código), en segundos. Rango válido: 0 < v <= 600. Perilla PROPIA y no la del chat a propósito: una coding tool tolera —y necesita— generaciones largas. Regla al girarla: el que ESPERA tiene que aguantar más que el que TRABAJA (el router del motor espera 120 s). |
| `SENTINEL_GW_BYOK_READ_TIMEOUT_SECONDS` | `150` | Timeout ENTRE CHUNKS del streaming BYOK de /gw, en segundos. Rango válido: 0 < v <= 600. El modelo local no manda pings mientras piensa: con un valor bajo la conexión se corta sola en mitad de una respuesta buena. Misma regla que el anterior. |

### ── Purga de retención: que el día 91 pase de verdad ─────────────────────────────────

La retención ya no es sólo una DECLARACIÓN: el DPO fija los plazos, la pantalla los muestra, el DPA los promete — y desde spec 018 (US1) hay un proceso que ELIMINA las filas vencidas de cada clase. Estas perillas lo configuran. Viene APAGADO por defecto (ver SENTINEL_PURGE_ENABLED): encenderlo es un acto explícito del operador. Las de la corrida se giran SIN reinstalar (se releen en cada corrida); la excepción es el interruptor maestro, que se lee al arrancar y por eso pide reinicio del backend. QUÉ ESTÁ PROTEGIDO Y QUÉ NO Los registros de la licencia —los que la caja escribe sola cada vez que la licencia se carga, entra en gracia o caduca— no se borran nunca, y ninguna perilla de esta lista los puede borrar. Lo que los protege es el SELLO que la propia caja deja escrito DENTRO de cada uno de esos registros; no los protege el nombre que figure en la columna «modelo». Todos los registros de licencia que la caja escribió alguna vez llevan ese sello, también los de instalaciones antiguas. Lo que NO está protegido: un pedido cualquiera puede declarar que su modelo se llama «license» —el nombre del modelo lo elige quien manda el pedido— y ese registro no lleva sello, así que es tráfico normal y se borrará cuando le toque por su plazo. Es a propósito: si bastara con llamarse «license» para no borrarse nunca, cualquiera con una clave de API se compraría un registro inmortal con sólo pedirlo así, que es justo lo que la retención viene a impedir. ⚠ ESTADO EN ESTA ENTREGA: el proceso que lee estas perillas —el que borra— YA viaja (spec 018 US1: purgador + scheduler cableados en el arranque del backend). Con SENTINEL_PURGE_ENABLED=false (el default) el scheduler ni arranca y no se borra ninguna fila, así que la caja sale configurada pero inerte. Ponerlo en `true` (con reinicio del backend) enciende la purga; hacerlo con SENTINEL_PURGE_DRY_RUN=true (el default) hace una corrida que CUENTA sin borrar, para que el DPO firme sobre el número antes del borrado real.

| Variable | Default | Descripción |
|---|---|---|
| `SENTINEL_PURGE_ENABLED` | `false` | Interruptor maestro del borrado por retención. En `false` (el default) el scheduler de purga ni arranca y no se elimina ninguna fila: la instalación queda configurada pero inerte. En `true` (con reinicio del backend — se lee al arrancar, no en caliente) el proceso arranca y, salvo simulacro (SENTINEL_PURGE_DRY_RUN), borra las filas vencidas de cada clase. Es la primera de la doble compuerta: el scheduler exige además un intervalo > 0. Viene APAGADO, y eso es una decisión, no un olvido: un proceso que borra hacia atrás no se enciende solo con una actualización de producto. La primera vez que arranque se encontraría con todo lo acumulado en la vida de la instalación y se lo llevaría de una — y lo borrado no vuelve. Encenderlo será un paso EXPLÍCITO del runbook, con reinicio del backend (se lee al arrancar, no en caliente) y después de haber leído el conteo del simulacro. La tensión con el Art. 5.1.e es real y está aceptada: una retención que todavía no borra se arregla con un `true`; un borrado que se encendió solo y se llevó de más no se arregla con nada. Se apaga también, ya en operación, para una instalación con un legal hold en curso. |
| `SENTINEL_PURGE_DRY_RUN` | `true` | Sólo surte efecto con SENTINEL_PURGE_ENABLED=true (ver arriba). ¿Simulacro? En `true` la corrida hará todo menos borrar: resuelve la fecha de corte, CUENTA las filas que caerían y deja el rastro marcado como simulacro. Nada se elimina. Es la segunda mitad de la red del encendido: aun con el interruptor en `true`, la instalación cuenta antes de borrar, y el DPO firma sobre un número real —«se van 412.000 filas de esta clase, con esta fecha de corte»— en vez de enterarse del alcance leyendo el rastro de lo ya borrado. Se pasa a `false` recién cuando ese número está mirado y aceptado, y es el único cambio de esta lista que hay que hacer una sola vez y con alguien mirando. |
| `SENTINEL_PURGE_INTERVAL_SECONDS` | `3600` | Sólo surte efecto con SENTINEL_PURGE_ENABLED=true (ver arriba). Cada cuánto DESPIERTA el proceso a mirar si está en ventana, en segundos. No es «cada cuánto borra»: si el momento cae fuera de SENTINEL_PURGE_WINDOW no hace nada y se vuelve a dormir. Bajarlo no borra más rápido, sólo engancha la ventana antes. |
| `SENTINEL_PURGE_WINDOW` | `02:00-05:00` | Sólo surte efecto con SENTINEL_PURGE_ENABLED=true (ver arriba). Ventana horaria en la que se permitirá borrar, `HH:MM-HH:MM` en la hora local de la instalación (ver SENTINEL_PURGE_WINDOW_TZ). El registro de auditoría es la tabla más caliente del producto —con el monitor, la analítica, los costes y el export leyendo encima— y el borrado compite con esos lectores: se trabaja de madrugada para que nadie note que existe. Una ventana que cruza medianoche (`23:00-02:00`) es válida. |
| `SENTINEL_PURGE_WINDOW_TZ` | `Europe/Madrid` | Sólo surte efecto con SENTINEL_PURGE_ENABLED=true (ver arriba). Zona horaria (nombre IANA) con la que se interpreta SENTINEL_PURGE_WINDOW. El contenedor corre en UTC: sin esto, «las 02:00» serían las 04:00 de la sede en verano y las 23:00 del día anterior en una instalación de LATAM — o sea, borrando en hora punta. |
...
Estas dos perillas son del DESPLIEGUE. Los tres datos del CLIENTE (Directory ID, Application ID y el secreto del directorio) no viven acá: van en la base, una fila por tenant, con el secreto cifrado con FERNET_SECRET_KEY. El SSO además exige que la licencia lo habilite; el acceso con usuario y contraseña sigue disponible siempre, y eso es permanente, no un provisorio. La operatoria completa —el registro en el directorio del cliente y sus trampas— está en la [guía de instalación de SSO](../install-deploy/sso.md).

| Variable | Default | Descripción |
|---|---|---|
| `SENTINEL_SSO_REDIRECT_URI` | — | URI de retorno del SSO. Apunta a la ruta de la CONSOLA, no a un endpoint de la API: el directorio redirige un NAVEGADOR, y es la consola la que canjea el código por la sesión con una llamada aparte, para que la sesión nunca viaje en la URL. Es OBLIGATORIA y EXPLÍCITA: sin valor el camino SSO corta con sso_redirect_uri_no_configurado y el login local sigue andando. Deducirla de la petición entrante la haría manipulable por cabecera. Tiene que ser la misma que se registró en el directorio del cliente, byte a byte —la barra final cuenta—, porque el mismo valor se usa al pedir el código y al canjearlo. |
| `SENTINEL_SSO_COOKIE_INSECURE` | — | SOLO desarrollo sobre HTTP plano: en 1, true o yes la cookie del flujo SSO viaja sin el atributo Secure. Vacía o ausente —el default— la cookie es Secure: en duda, no viaja en texto claro. En producción se deja vacía. |
