"""basa_governance — catálogo de capas y resolutor PURO de gobernanza (spec 027).
...
**Camino de import canónico**: ``from extensions import basa_governance`` — se agrega a
...
alias ``sys.modules['basa_governance']`` como red de seguridad, para que un caller que
...
**Por qué un módulo nuevo y no dentro de ``basa_guardian_policy``**: hay TRES planos que
...
``basa_guardian_policy.py`` en curso. El backend lo re-exporta delgado desde
...
_LOG = logging.getLogger("basa.governance")
...
class Profile:
    ...
    def to_dict(self) -> dict:
        ...
        resuelto en ``metadata['basa']``, no lee estas tablas: leerlas desde el motor es
    ...
    def from_dict(cls, data: Optional[dict], *, trusted: bool = False) -> "Profile":
        ...
        El perfil cruza al motor por ``metadata['basa']``, o sea dentro de un body que el
        ...
        **Requisito duro para el caller del ingress (T024/US2)**: antes de inyectar el
        perfil propio debe **BORRAR** ``metadata['basa']`` entrante. Sin ese borrado, un
...
# Camino canónico: `from extensions import basa_governance` (en sys.path va la carpeta
...
# (`from basa_governance import ...`, el mecanismo histórico de gateway.py). Sin este
...
sys.modules.setdefault("basa_governance", _THIS_MODULE)
sys.modules.setdefault("extensions.basa_governance", _THIS_MODULE)
_EXT_PKG = sys.modules.get("extensions")
if _EXT_PKG is not None and not hasattr(_EXT_PKG, "basa_governance"):
    setattr(_EXT_PKG, "basa_governance", _THIS_MODULE)
