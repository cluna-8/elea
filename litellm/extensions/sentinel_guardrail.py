"""SentinelGuardrail — la política Sentinel como CustomGuardrail nativo (spec 014 US1).
...
def _sentinel_identity(user_api_key_dict) -> dict:
    md = getattr(user_api_key_dict, "metadata", None) or {}
    return md.get("sentinel") or {}
...
# ya usa `sentinel_audit_logger` para el camino de éxito; acá se estrena el emisor de BLOQUEO.
...
# Capas del registry 027 (`sentinel_governance.LAYER_KEYS`) por punto de bloqueo. Se escriben
...
# (sentinel_audit_logger.py) cuando la identidad no resuelve. Un bloqueo sin tenant NO puede
...
def _audit_fail_mode() -> str:
    """`open` | `closed` desde `SENTINEL_AUDIT_FAIL` (contrato §Config).
    ...
    return (_AUDIT_FAIL_CLOSED
            if os.environ.get("SENTINEL_AUDIT_FAIL", "").strip().lower() == _AUDIT_FAIL_CLOSED
            else _AUDIT_FAIL_OPEN)


def _audit_url() -> str:
    """URL del plano interno de auditoría (`SENTINEL_AUDIT_URL`), o cadena vacía.
    ...
    return os.environ.get("SENTINEL_AUDIT_URL", "").strip()
...
async def _contar_perdida(motivo: str) -> None:
    ...
    client = None
    try:
        client = sentinel_engine_redis.async_redis_con_timeouts(redis_lib, *_redis_endpoint())
        ...
        # cliente filtrado. Mismo patrón que `sentinel_audit_logger._registrar_perdida`.
...
async def _emitir_fila(entry: dict) -> bool:
    ...
    if not url:
        logger.error(
            "audit: SENTINEL_AUDIT_URL no configurada — el plano motor no puede registrar el "
...
async def _auditoria_escribible() -> bool:
    ...
    url = _audit_url()
    if not url:
        logger.error("audit: SENTINEL_AUDIT_FAIL=closed sin SENTINEL_AUDIT_URL — el plano motor "
...
async def _auditar_bloqueo(user_api_key_dict, data: dict, *, compliance_status: str,
    ...
    (ver `_attribution_del_motor` en sentinel_audit_logger.py); `blocked_by_layer` sí es
    afirmable —lo decidió ESTE código, en este proceso— y es lo que el contrato exige
    obligatorio en un bloqueo.
    """
    try:
        identidad = _sentinel_identity(user_api_key_dict)
        entry = {
...
#   1. `sentinel_compliance.status = degraded_nlp_regex` → lo copia el emisor de éxito
#      (`sentinel_audit_logger`, `compliance = request_md["sentinel_compliance"]["status"]`), así que
#      la fila DURABLE de esa transacción lo lleva sin tocar el logger;
#   2. marca de estado en Redis (`sentinel:nlp:degraded_*`) → la lee `GET /api/v1/health` y la
...
_REDIS_KEY_NLP_DEGRADED_SINCE = "sentinel:nlp:degraded_since"
_REDIS_KEY_NLP_DEGRADED_COUNT = "sentinel:nlp:degraded_requests"
...
async def _marcar_nlp_degradado() -> None:
    ...
    try:
        ...
        # (`_contar_perdida` → `sentinel:audit:lost` + /health), no un mecanismo nuevo.
...
class SentinelGuardrail(CustomGuardrail):
    """La política de compliance de Sentinel Guardian, montada en el motor."""
