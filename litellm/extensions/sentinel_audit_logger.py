"""SentinelAuditLogger — auditoría metadata-only desde el motor (spec 014 US3).
...
CUENTA en Redis (``sentinel:audit:lost`` / ``sentinel:audit:last_fail``) para que el health y el
banner de Logs de Auditoría la muestren. La política ``SENTINEL_AUDIT_FAIL`` se lee para el
...
# Mismo patrón que las otras extensiones del motor (`sentinel-guardrail`, `sentinel-custom-auth`):
...
# esquema que el productor del gateway. El comentario "MISMO esquema que sentinel_audit_logger"
...
_REDIS_KEY_AUDIT_LOST = "sentinel:audit:lost"
_REDIS_KEY_AUDIT_LAST_FAIL = "sentinel:audit:last_fail"
_AUDIT_FAIL_ENV = "SENTINEL_AUDIT_FAIL"
...
# `sentinel_guardrail.py`: escribir las mismas claves en OTRO host es exactamente la divergencia
...
async def _registrar_perdida(motivo: str) -> None:
    ...
    try:
        ...
        client = sentinel_engine_redis.async_redis_con_timeouts(
...
async def _postear_al_plano_interno(audit_url: str, entry: dict) -> bool:
    ...
    # SENTINEL_ENGINE_MASTER_KEY: renombrar acá manda "" y la fila se rechaza.
    headers = {"X-Sentinel-Internal": os.environ.get("LITELLM_MASTER_KEY", "")}
...
async def emitir_fila_durable(entry: dict, masked: list,
    ...
    si hay ``SENTINEL_AUDIT_URL`` (producción), o por el prisma del motor en desarrollo.
    ...
    audit_url = os.environ.get("SENTINEL_AUDIT_URL", "").strip()
    if audit_url:
        await _postear_al_plano_interno(audit_url, entry)
    else:
        await sentinel_audit_logger_instance._insertar_por_prisma(
...
def _attribution_del_motor():
    ...
    **Por qué no se lee del metadata-home** (hallazgo A3 de la verificación adversarial de
    ...
    ``metadata['sentinel_governance']``. Ese home es la fusión de ``litellm_metadata`` y
    ...
    *vocabulario*, no *procedencia*: un cliente con virtual key mandando
    ``{"metadata": {"sentinel_governance": {"applied_layers": [{"layer_code": "pii_masking",
    ...
    **Estado del cableado (2026-07-22)**: el productor propio es **T025**
    (``sentinel_guardrail.py``), bloqueada por el PR #21, que reescribe el guardrail entero.
...
class SentinelAuditLogger(CustomLogger):
    ...
    async def _log(self, kwargs, response_obj, start_time, end_time):
        ...
        # La identidad Sentinel viaja en el UserAPIKeyAuth.metadata (custom_auth) que el
        ...
        sentinel = policy.proxy_identity_from(data) or policy.proxy_identity_from(kwargs)
        ...
        if not (sentinel and (sentinel.get("key_id") or sentinel.get("client_id"))):
        ...
        masked = request_md.get("sentinel_masked_entities") or []
        compliance = (request_md.get("sentinel_compliance") or {}).get("status") or "passed"
        ...
        # separación de bases (motor → sentinel_engine) el INSERT por prisma fallaba en cada
        ...
        # (`sentinel:audit:lost` + `sentinel:audit:last_fail`) y se loguea con nivel error. La
        # misma pérdida hoy se vería en minutos en el health y en el banner de Logs.
        entry = {
            "tenant_id": sentinel.get("tenant_id") or "00000000-0000-0000-0000-000000000001",
            "user_id": sentinel.get("client_id"),
            "api_key_id": sentinel.get("key_id"),
            ...
            "user_group_id": sentinel.get("group_id"),
        ...
        await self._publish_monitor_event(sentinel, masked, compliance, kwargs,
                                          applied_layers, blocked_by_layer)

    async def _insertar_por_prisma(self, entry, masked, applied_layers, blocked_by_layer):
        """Camino de DESARROLLO (base compartida, sin `SENTINEL_AUDIT_URL`): INSERT directo.
        ...
        # la auditoría de un motor sin DATABASE_URL aunque `SENTINEL_AUDIT_URL` estuviera bien
        # cableada. Otro fallo mudo menos.
        from litellm.proxy.proxy_server import prisma_client
        if prisma_client is None:
            logger.error("no se pudo auditar: el motor no tiene ni SENTINEL_AUDIT_URL ni "
    ...
    async def _publish_monitor_event(self, sentinel, masked, compliance, kwargs,
        ...
        try:
            ...
            event = {
                ...
                "tool": sentinel.get("tool_type") or sentinel.get("ua_tool"),
                "client": sentinel.get("client_username"),
                "tenant": sentinel.get("tenant_slug"),
            ...
            if client is not None:
                try:
                    ...
                    pass
sentinel_audit_logger_instance = SentinelAuditLogger()
