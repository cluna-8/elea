...
# Necesita el compose de dev arriba (basa-db); usa una DB efímera propia.
set -euo pipefail
...
grep -q "PRODUCT_DOMAIN=example.clients.basa.example" "$R/instance.env" || fail "dominio no deriva del slug"
...
cd "$REPO_ROOT"
docker exec basa-db psql -U basa_admin -d basa_gateway -q -c "DROP DATABASE IF EXISTS basa_profile_check" 
docker exec basa-db psql -U basa_admin -d basa_gateway -q -c "CREATE DATABASE basa_profile_check"
trap 'docker exec basa-db psql -U basa_admin -d basa_gateway -q -c "DROP DATABASE IF EXISTS basa_profile_check"' EXIT

run_seed() {
  docker compose run --rm --no-deps -e POSTGRES_DB=basa_profile_check \
...
count=$(docker exec basa-db psql -U basa_admin -d basa_profile_check -tAc \
