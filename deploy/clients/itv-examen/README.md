...
2. **`compose.itv-examen.override.yml`** — cablea `BASA_GW_ANTHROPIC_BASE` (passthrough de
...
| `client.env.example` | Overlay de entorno (STUB_URL, keys vacías, `BASA_ALLOW_DEV_LICENSE`, imágenes). `*.env` está gitignored → se versiona el `.example`. |
...
| `compose.itv-examen.override.yml` | Cablea `BASA_GW_ANTHROPIC_BASE` → stub. |
...
`basa-dev-2026b`**. La **clave privada de ese kid NO está en el repo** (`license_out/` es
...
```bash
# 300 seats — gates 125 y 250 (PENDIENTE: requiere signing_key_basa-dev-2026b.pem de JF)
...
- La `.lic` emitida se monta como `/app/config/licenses/client.lic` (con
  `BASA_ALLOW_DEV_LICENSE=true`, ya en `client.env`). **Guardala FUERA del repo.**
...
del `expected_tenant_id()` (`BASA_DEPLOYMENT_TENANT_ID`), mientras que el seat gate real de
...
`BASA_DEPLOYMENT_TENANT_ID=00000000-0000-0000-0000-000000000001` (= DEFAULT): con eso
