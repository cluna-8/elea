...
rechazando con un 503 rápido (``X-Sentinel-Rejected: saturated``), el examen mide TAMBIÉN esa
...
def eval_saturated_durable(k6_summary: dict, reconciliation: dict) -> SLOResult:
    ...
    El guion cuenta un rechazo por cada 503 con ``X-Sentinel-Rejected: saturated`` (el header
    ...
    detalle = {"rechazos_guion": rechazos, "filas_rejected_saturated": filas,
               "fuente": "503 con X-Sentinel-Rejected: saturated (guion) vs audit_logs con "
    ...
    if _is_int(filas) and filas > 0 and (rechazos is None or rechazos == 0):
        detalle["nota"] = (f"el producto tiene {filas} fila(s) rejected_saturated pero el "
                           ...
                           "header X-Sentinel-Rejected? El detector de rechazos está ciego")
...
def eval_instrument(k6_summary: dict, stub_report: dict) -> dict:
    ...
    # sentinel_key): carga programada que desapareció del examen. Como bajan a la vez los
    ...
    if tiene_herrores and herrores != 0:
        razones.append(f"harness_errors={herrores}: iteraciones que no ejecutaron su "
                       "superficie (¿pool sin sentinel_key? ¿login storm contra población "
