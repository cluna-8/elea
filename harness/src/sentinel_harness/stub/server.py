...
def create_app(state: Optional[StubState] = None) -> FastAPI:
    ...
    app = FastAPI(title="sentinel-harness stub", docs_url=None, redoc_url=None)
    app.state.stub = st
...
# Instancia por defecto para ``uvicorn sentinel_harness.stub.server:app`` (serving real). El
