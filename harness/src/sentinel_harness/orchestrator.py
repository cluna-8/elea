...
``sentinel_key`` de las Connections —sin ellas las superficies extensión/coding no autentican
...
class Orchestrator:
    ...
    def _export_identities(self) -> list[dict]:
        ...
        El material de llave (``sentinel_key``) NO se puede derivar offline: la key en claro la
        ...
        for m in members:
            pool.append({"username": m.username, "password": m.password, "role": m.role,
                         ...
                         "sentinel_key": material.get(m.username)})
    ...
    def _key_material(self) -> dict:
        """``username → sentinel_key`` del pool del seeder (vacío si no hay ``--pool-file``)."""
        if self.pool_file is None:
            return {}
        out = {}
        for entry in self._read_pool_file():
            if isinstance(entry, dict) and isinstance(entry.get("sentinel_key"), str) \
                    and entry["sentinel_key"]:
                out[entry.get("username")] = entry["sentinel_key"]
        return out
    ...
    def _assert_key_material(self, members: list, material: dict) -> None:
        ...
        if faltan:
            raise OrchestratorError(
                f"{len(faltan)} identidad(es) de extensión/coding sin sentinel_key en "
    ...
    def _synthetic_summary(self) -> dict:
        ...
        for surface in SURFACE_EXEC:
            ...
            if surface == "coding_sse":
                ...
                entry["stream_cuts"] = 0
            surfaces[key] = entry
        summary = {"schema": "sentinel-harness/k6-summary@1", "gate": self.gate.gate,
...
def _default_smoke(backend_url: str, model_chat: str,
    ...
    def smoke(pool: list) -> list:  # pragma: no cover — camino real (los tests inyectan)
        ...
        def _key(client_type):
            for c in pool or []:
                if isinstance(c, dict) and c.get("client_type") == client_type \
                        and c.get("sentinel_key"):
                    return c["sentinel_key"]
            return None
        ...
        with httpx.Client(timeout=60.0) as http:
            ...
            k_ext = _key("desktop")
            if k_ext is None:
                fallos.append(("extension", "el pool no trae sentinel_key de client_type=desktop"))
            else:
                r = http.post(f"{base}/gw/inspect", json={"text": TEXTO, "tool": "chatgpt"},
                              headers={"X-Sentinel-Key": k_ext})
                if r.status_code != 200:
                    fallos.append(("extension", f"HTTP {r.status_code} → {r.text[:160]}"))

            k_cod = _key("base_url")
            if k_cod is None:
                fallos.append(("coding", "el pool no trae sentinel_key de client_type=base_url"))
            else:
...
def main(argv: Optional[list] = None) -> int:
    p = argparse.ArgumentParser(
        prog="python -m sentinel_harness.orchestrator",
    ...
    p.add_argument("--pool-file", type=Path, default=None,
                   help="pool 0600 emitido por `seeder.seed --emit-credentials`: aporta "
                        "las sentinel_key de las Connections (extensión/coding) y la "
