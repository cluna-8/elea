...
- ``GET  /api/v1/gw/whoami``       valida una key en claro → identidad, con el header
  ``X-Sentinel-Key``; fail-closed 401 si no resuelve (``backend/src/api/inspect.py:174``).
...
class BackendClient:
    ...
    def verify_sentinel_key(self, sentinel_key: str) -> bool:
        """``GET /gw/whoami`` con ``X-Sentinel-Key``: True si la key resuelve a una identidad.
        ...
        resp = self._http.request("GET", url, headers={"X-Sentinel-Key": sentinel_key})
        if resp.status_code == 200:
            return True
        if resp.status_code == 401:
            return False
