...
| 016 | Real NLP Masking (Presidio) + streaming unmask hardening | Cristian | P2 | **Lista para merge** ([PR #21](https://github.com/DrZuzzjen/basa-guardian/pull/21) — suite completa (incl. `tests/integration/`) y `make check-docs` verdes tras rename `NLP_ANALYZER_URL`/servicio `nlp-analyzer`; falta commitear+pushear el rename y decidir rebase vs merge) | I, SC-2 | **C2** |
