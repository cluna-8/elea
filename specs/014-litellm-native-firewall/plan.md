...
1. **Motor LiteLLM** (ruta BYOK + guardrails + audit): la política Sentinel vive en tres puntos de
   extensión documentados — `SentinelGuardrail` (CustomGuardrail: pre/post/streaming), `custom_auth`
   (identidad), `SentinelAuditLogger` (CustomLogger: audit metadata-only) — sobre el endpoint
...
2. **Backend** (única excepción del Principio VI): el passthrough OAuth de **suscripción**, porque
   ...
   `sentinel_guardian_policy` que el guardrail, para no divergir.

Ambos hogares comparten una **librería pura** `sentinel_guardian_policy` (mask/unmask/detect/carry-split).
El enfoque técnico central: **reuso del motor** (Principio VI) — sólo la política Sentinel es código propio;
...
`/v1/messages`; integration tests por user story; unit tests de `sentinel_guardian_policy` (carry-split,
...
| **Dev Workflow — Reuse over Reinvent** | El port borra ~470 líneas hand-rolled y delega al motor; sólo la policy Sentinel es propia. | PASS by-design / a verificar |
...
| `_extract_inspect_text` + `ComplianceService.evaluate_prompt` + `GuardianService.process_prompt` (bloqueo) | `SentinelGuardrail.async_pre_call_hook` | PROPIO (wrapper) |
| `_redact_body` (mask reversible con nonce) | `async_pre_call_hook` → `sentinel_guardian_policy.mask_reversible`; mapa en `data["metadata"]["pii_tokens"]` | PROPIO (policy) |
...
| `_safe_split` / `_PH_TYPE_RE` / `_PH_TAIL_RE` | `sentinel_guardian_policy` (carry-split) | PROPIO (policy) |
| `_detect_tool` / `_TOOL_UA` | `custom_auth.user_api_key_auth` (UA→tool_type) | PROPIO (resolución) |
| `_resolve_identity` (X-Sentinel-Key → user/group) | `custom_auth` (virtual key → tenant/client/team vía `APIKey`) | PROPIO (resolución) sobre `UserAPIKeyAuth` del motor |
| `_log_tx` → `AuditService.log_transaction` | `SentinelAuditLogger.async_log_success_event` + scrub `pii_tokens` | PROPIO (wrapper), REUSA `AuditService` |
...
| modo `subscription-passthrough` (013; "anthropic" en el demo) — OAuth passthrough | thin reverse-proxy en backend + `sentinel_guardian_policy` | PROPIO (única excepción VI) |
...
└── extensions/                            # paquete de extensiones Sentinel (nuevo)
    ├── sentinel_guardian_policy.py            # PROPIO: librería pura (mask/unmask/detect/carry-split) — compartida
    ├── sentinel_guardrail.py                  # PROPIO: SentinelGuardrail(CustomGuardrail) — 3 hooks
    ├── custom_auth.py                     # PROPIO: user_api_key_auth (identidad, fail-closed)
    └── sentinel_audit_logger.py               # PROPIO: SentinelAuditLogger(CustomLogger) — audit + feed monitor

backend/src/
├── api/
│   ├── gateway.py                         # ADELGAZADO: sólo passthrough OAuth de suscripción sobre sentinel_guardian_policy
...
└── unit/                                  # sentinel_guardian_policy: carry-split, colisión, round-trip
...
única excepción (passthrough OAuth) y la vitrina. La librería pura `sentinel_guardian_policy` es importable
...
1. **Fundacional — librería pura + pin.** `sentinel_guardian_policy` (mask/unmask/detect/carry-split,
...
3. **US1 firewall nativo (P1).** `SentinelGuardrail` con los tres hooks importando la policy; registro en
   `guardrails:` con los tres modes. MVP demostrable: block + mask + unmask (no-streaming y streaming).
4. **US3 audit + monitor (P2).** `SentinelAuditLogger` (metadata-only + scrub) en
...
5. **US4 excepción suscripción (P2).** Adelgazar `gateway.py` a sólo el passthrough OAuth, reescrito
   sobre `sentinel_guardian_policy`; borrar `/gw/v1/messages`, count_tokens, models, `_rewrite_sse_event`,
...
| **Ambigüedad de identidad en suscripción sin `X-Sentinel-Key`** (Constraint C3 vs modo sin key). | Medio — o se rompe la demo, o se abre un agujero. | **Resuelto por [D-014]** (default revisable): fail-closed duro sólo en `byok`; en `subscription-passthrough` la credencial es el OAuth y `X-Sentinel-Key` es atribución opcional (si falta, `tenant-default` anónimo, auditado como tal). US4 desbloqueado. |
| **Provisioning virtual keys ↔ APIKey Sentinel** (doble fuente de verdad). | Medio — identidad inconsistente. | Resolución directa por `key_hash` contra `APIKey` en `custom_auth` (mantiene el modelo de identidad en 013). |
...
- **Estrategia A — hook sobre objetos parseados (`ModelResponseStream`)**: el motor entrega chunks ya
  ...
  (text/thinking/tool_use.input_json), aplica unmask y mantiene el carry-split de `sentinel_guardian_policy`.
