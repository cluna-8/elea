...
1. Un **artefacto de LICENCIA FIRMADO** verificable **OFFLINE** (token **Ed25519** con `tenant_id`,
   `max_seats`, `expiry`, `feature_flags`, `license_id`), chequeado al **arranque** y en la **creación**
   de Connection/Client — **NADA de license-server / phone-home** (on-prem/VPN puede no tener egress).
...
producto embebe). El **firmante** (clave privada) vive del lado de Basa/distribuidor, offline.
...
Basa (o el distribuidor autorizado) emite **offline** un **token de licencia firmado con Ed25519** que
contiene `license_id`, `tenant_id`, `max_seats`, `issued_at`, `expiry`, `feature_flags` y `grace_days`.
...
1. **Given** un token de licencia firmado Ed25519 válido para el `tenant_id` del deployment, **When** el
   ...
   `{license_id, tenant_id, max_seats, expiry, feature_flags, grace_days}` en memoria y queda operativo.
...
### User Story 5 - Evidencia de tamper vía audit inmutable (Priority: P3)
...
**audit log inmutable YA existente** (metadata-only). Como el audit es append-only, esos eventos son
**evidencia de tamper** que sobrevive incluso si alguien manipula la caja: un auditor puede reconstruir si
el cliente corrió over-seat, expirado, o con un token inválido. NO se inventa un canal nuevo: se **reusa**
la infraestructura de audit inmutable.

**Why this priority**: En un modelo donde el cliente **corre el software**, el enforcement no puede ser
inviolable — pero **sí puede ser innegable**. El audit inmutable convierte cualquier tamper en un rastro
verificable (evidencia para el contrato / disputa comercial). Es P3 porque el enforcement funciona sin
...
**ninguno** puede borrarse/editarse por la ruta normal (inmutabilidad).
...
1. **Given** el audit inmutable existente, **When** ocurre cualquier transición de licencia (carga,
   ...
   metadata-only con `{event_type, license_id, tenant_id, seats_used, max_seats, ts}` y **cero** secretos
   (ni la clave, ni el token crudo).
...
- **FR-001**: El sistema MUST definir un **artefacto de licencia** firmado con **Ed25519** que contenga al
  menos `{schema, license_id, tenant_id, max_seats, issued_at, not_before, expiry, grace_days,
  feature_flags, key_id}`. El formato wire `.lic` (JSON canónico + firma detached; nombres compactos
  `lic_id`/`kid`) y el veredicto build-vs-buy (DIY Ed25519) están fijados en [`research.md`](./research.md).
- **FR-002**: El producto MUST embeber **sólo la(s) clave(s) PÚBLICA(s)** de Basa; la clave privada
  (firmante) NUNCA vive en el contenedor ni en la caja del cliente (vive offline del lado de Basa/distribuidor).
...
- **FR-026**: El token de licencia MUST inyectarse como **config** del artefacto de deploy de la 020
  ...
  licencia es config firmada, no un binario custom).
...
- **LicenseToken (artefacto firmado)** — *NUEVO, código PROPIO*. Payload firmado Ed25519 con
  `{schema, license_id, tenant_id, max_seats, issued_at, not_before, expiry, grace_days, feature_flags,
  key_id}` + firma (esbozo `.lic` completo en [`research.md`](./research.md); wire `lic_id`/`kid`).
  Se **inyecta como config** (020), se verifica offline al arranque. NO se persiste crudo en audit.
...
- **El firmante vive offline del lado de Basa/distribuidor**: la clave privada Ed25519 NUNCA se despliega
  en la caja del cliente (custodia en KMS/HSM del lado Basa, ver `research.md`). El producto sólo verifica;
  no firma.
...
- **Enforcement offline = detectable, no inviolable**: como el cliente **corre la caja**, el enforcement
  no puede ser criptográficamente inviolable end-to-end (podrían parchear el binario). El objetivo
  realista es **fail-closed + anti-tamper auditable**: dificultar la evasión y dejar **evidencia
  innegable** (audit inmutable, US5) para el contrato/disputa comercial. Esto es explícito, no una carencia
  oculta.
