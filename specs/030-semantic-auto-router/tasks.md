...
- [ ] T001 Motor: entrada `router-embeddings` → `ollama/qwen3-embedding:0.6b` en
...
- [ ] T002 [P] Seeds: `litellm/auto_router.json` (dev — 3 rutas schema data-model.md §1,
...
- [ ] T003 `backend/src/services/auto_router_service.py` — port adaptado de
...
- [ ] T004 [P] Unit tests `backend/tests/unit/test_auto_router_service.py`: switch OFF sin
...
- [ ] T005 Migración `backend/alembic/versions/013_auto_router_decision.py` (columna
...
- [ ] T006 [US1] `backend/src/api/chat.py`: si `request.model == "auto"` → decisión al
...
- [ ] T007 [P] [US1] `GET /chat/models` (chat.py:1278): antepone pseudo-modelo «auto»
      cuando `enabled` (contrato §GET /models); excluye `router-embeddings` y el
      pseudo-modelo de cualquier flujo de gestión.
- [ ] T008 [P] [US1] Vitrina: kwarg opcional `routing` en `_publish_monitor`
...
- [ ] T009 [US1] Integration tests `backend/tests/integration/test_chat_auto_router.py`
...
- [ ] T010 [US1] **CHECKPOINT VIVO** (stack camara, quickstart): 3 prompts→3 destinos
...
- [ ] T011 [US2] `backend/src/api/router_config.py` — GET/PUT `/chat/router-config`
...
- [ ] T012 [P] [US2] Tests `backend/tests/integration/test_router_config_api.py`:
...
- [ ] T013 [US2] Frontend: `getRouterConfig/putRouterConfig` en
...
- [ ] T014 [P] [US2] `frontend/src/pages/PlaygroundPage.tsx` + `UserPortal.tsx`: «Auto»
...
- [ ] T015 [US2] **CHECKPOINT VIVO**: switch OFF por panel → SC-003 (cero llamadas en logs
...
- [ ] T016 [US3] `register_model` (chat.py:1306): al dar de alta un modelo NO-ollama con
...
- [ ] T017 [P] [US3] Edge /gw: `model=="auto"` en byok → reescribir al default_model antes
      de reenviar al motor (gateway.py camino byok ~:801, research R9) + test.
- [ ] T018 [US3] **CHECKPOINT VIVO**: romper api_base de un cloud + restart motor →
...
- [ ] T019 [P] Docs: `deploy/release/INSTALL-CAMARA.md` (+`ollama pull
...
- [ ] T020 Gate final: suite backend completa (verde salvo los 3 preexistentes conocidos),
