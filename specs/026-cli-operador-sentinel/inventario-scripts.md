...
- **`generate_trueup.py` vuelca el JSON firmado a stdout** (si falta el redirect se pierde); envío a Sentinel fuera de banda sin tracking. -> FR-009/FR-013.
...
- **[RESUELTA] Dónde vive el ledger del cupo/seats/génesis mientras no hay portal**
  - **Resolución**: ledger local **JSONL append-only con hash-encadenado** (lado Sentinel),
  ...
  - **Fuente**: research.md D5; data-model.md § "Ledger local de emisiones (lado Sentinel)".
...
- **[RESUELTA] Empaquetado y distribución de la propia CLI**
  - **Resolución**: **dos artefactos físicamente separados** — `sentinel-admin-signer`
    (interno Sentinel, estación de firma aislada, nunca se entrega a partners/clientes ni
    viaja en el bundle) y `sentinel-admin` (partner/cliente, único artefacto dentro del
