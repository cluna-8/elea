# Quickstart — validación viva de la 026 (`sentinel-admin-signer` + `sentinel-admin`)
...
make -C cli build-signer                     # single-arch, estación de firma de Sentinel
make -C cli build-admin                      # por arquitectura x86_64/arm64
ls cli/sentinel_admin_signer/dist/sentinel-admin-signer-*.pyz
ls cli/sentinel_admin/dist/sentinel-admin-*.pyz
...
#    → pytest cli/sentinel_admin_signer/tests/ (unit + CliRunner) Y el e2e:
#      el .pyz emite un .lic → docker compose run backend valida con verify_license_blob
make -C cli test-artifact-boundary-admin
#    → abre sentinel-admin-*.pyz (zipfile) y FALLA el build si aparece cualquier módulo/
#      símbolo/dependencia exclusivo del signer (keycustody/signing/ledger de emisiones)

# 2) Emitir una licencia dev por flags — SOLO con sentinel-admin-signer,
#    en la estación de firma (sin editar código, sin tocar el keyset)
KEYSET_ANTES=$(sha256sum backend/src/keys/sentinel_public_keys.pem)
./cli/sentinel_admin_signer/dist/sentinel-admin-signer-*.pyz license issue-dev \
    --tenant <TENANT_ID> --seats 10 --expiry 2027-01-01 --out /tmp/demo.lic  # passphrase por prompt
sha256sum backend/src/keys/sentinel_public_keys.pem      # DEBE ser == $KEYSET_ANTES (SC-002)
...
./cli/sentinel_admin_signer/dist/sentinel-admin-signer-*.pyz license verify /tmp/demo.lic
docker compose run --rm --no-deps backend python -c \
  "from src.licensing.verifier import verify_license_blob, SentinelPublicKeySet; \
   print(verify_license_blob(open('/tmp/demo.lic').read(), \
     SentinelPublicKeySet.from_pem_file('src/keys/sentinel_public_keys.pem')).license_id)"

# 4) Secuencia de instalación guiada contra el stack dev (SC-001) —
#    SOLO con sentinel-admin, en el host del partner/cliente. El .lic firmado en (2)
#    viaja como ARCHIVO fuera de banda hasta acá (nunca sentinel-admin-signer).
sentinel-admin install profile render <slug>        # valida layout + vars, error legible
sentinel-admin install seed apply --dry-run ...     # muestra diff (incl. rename tenant), no commitea
sentinel-admin install seed apply ...               # confirma mostrando a qué DB apunta; keys a archivo 600
sentinel-admin install license verify /tmp/demo.lic # sanity offline previo a instalar
sentinel-admin install license install /tmp/demo.lic   # puebla el volumen + restart + poll /health/license
sentinel-admin install admin bootstrap --username admin   # password por prompt (SC-004; mata el TOFU raceable)
sentinel-admin install verify <slug>                # smoke e2e en verde (licencia activa + gateway + masking)
...
make -C cli test-offline-admin    # ídem, paquete sentinel_admin

# 6) Bundle air-gapped: layout completo de FR-021, SOLO sentinel-admin viaja adentro — sentinel-admin-signer NUNCA
sentinel-admin install bundle create <slug> --from-lockfile release.lock --out /tmp/bundle
tar -tzf /tmp/bundle/bundle-v*.tar.gz > /tmp/bundle_listing.txt
grep -E '^bin/' /tmp/bundle_listing.txt
#    → esperado EXACTAMENTE: bin/sentinel-admin-x86_64.pyz, bin/sentinel-admin-arm64.pyz
...
! grep -q 'sentinel-admin-signer' /tmp/bundle_listing.txt
#    → escaneo del tarball COMPLETO (no solo bin/): si aparece sentinel-admin-signer en
#      CUALQUIER ruta, el build está roto — FR-021/FR-023

# 7) Instalar sentinel-admin en un host limpio usando SOLO install.sh (FR-023: nunca pip/apt/yum)
...
física; (2) keyset intacto tras emitir con `sentinel-admin-signer`; (3) el mismo `.lic` valida
en ambos artefactos y en el producto; (4) instalación completa con `sentinel-admin` sin editar
scripts ni SQL, con confirmaciones visibles, sin haber tocado `sentinel-admin-signer`; (5)
núcleo de ambos verificado sin red; (6) el bundle tiene el layout completo de FR-021 y
solo contiene `sentinel-admin` en cualquier ruta; (7) `install.sh` deja `sentinel-admin` listo
