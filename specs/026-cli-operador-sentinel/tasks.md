# Tasks: CLI de operador (`sentinel-admin-signer` + `sentinel-admin`) — firma de licencias e instalación guiada offline

**Input**: Design documents from `/specs/026-cli-operador-sentinel/`
...
`sentinel-admin-signer`; US2 (instalación) vive **exclusivamente** en `sentinel-admin`. Ningún
task de US1 toca `cli/sentinel_admin/` y ningún task de US2 toca `cli/sentinel_admin_signer/` —
...
- [ ] T001 Crear el paquete `cli/sentinel_admin_signer/` (`pyproject.toml` con deps
      click+cryptography, entry point `sentinel-admin-signer`; `sentinel_admin_signer/__init__.py`;
      `__main__.py` con el grupo click raíz y `--version`) según el árbol del plan.md.
      `pyproject.toml` **no** declara dependencia hacia `sentinel_admin` (FR-016)
- [ ] T002 Crear el paquete `cli/sentinel_admin/` (`pyproject.toml` con deps
      click+cryptography, entry point `sentinel-admin`; `sentinel_admin/__init__.py`;
      `__main__.py` con el grupo click raíz y `--version`) según el árbol del plan.md.
      `pyproject.toml` **no** declara dependencia hacia `sentinel_admin_signer` (FR-016/FR-018)
- [ ] T003 [P] `cli/sentinel_admin_signer/Makefile`: targets `build` (shiv → single-arch
      `dist/sentinel-admin-signer.pyz` para la estación de firma de Sentinel, `SHIV_ROOT` fijado —
      research D3), `test`, `test-offline` (pytest con `--network none`), `test-contract`
- [ ] T004 [P] `cli/sentinel_admin/Makefile`: targets `build-x86_64` y `build-arm64` (shiv →
      `dist/sentinel-admin-x86_64.pyz` / `dist/sentinel-admin-arm64.pyz` por separado, wheels de
...
- [ ] T006 [P] `cli/sentinel_admin_signer/sentinel_admin_signer/ui.py`: helpers `confirm()` (con
      ...
      2/3/4/6). Implementación propia, sin importar nada de `sentinel_admin`
- [ ] T007 [P] `cli/sentinel_admin/sentinel_admin/ui.py`: los mismos helpers que T006
      (`confirm()`, prompt sin eco, exit codes 0-4), implementación **propia e
      independiente** — ningún import cruzado hacia `sentinel_admin_signer` (FR-019/FR-020)
- [ ] T008 [P] `cli/README.md`: documentar la frontera de import selectivo de
      ...
      `deployment_key.py`/`trueup_export.py` son elegibles para `sentinel_admin` (no tocan la
      privada del emisor); `keycustody.py`/`signing.py`/`ledger.py` de emisiones son
      código nuevo, exclusivo de `sentinel_admin_signer`, y **nunca** deben importarse desde
      `sentinel_admin`
...
- [ ] T009 [P] `cli/sentinel_admin_signer/sentinel_admin_signer/keycustody.py`: crear/importar/
      ...
      Este módulo **nunca** existe fuera de `sentinel_admin_signer/` (FR-017)
- [ ] T010 [P] Tests unit de keycustody en
      `cli/sentinel_admin_signer/tests/test_keycustody.py`: no pisa clave existente (O_EXCL),
      permisos, round-trip cifrado→firma, import valida kid, passphrase equivocada → error
      legible exit 1
- [ ] T011 [P] `cli/sentinel_admin_signer/sentinel_admin_signer/ledger.py`: writer JSONL
      append-only hash-encadenado genérico, instanciado dos veces dentro del paquete: (a)
      ledger de emisiones de licencias y (b) audit-log local de `sentinel-admin-signer`
      (comando/args sanitizados/resultado, metadata-only), con verificación de cadena y
      export (research D5) + tests en
      `cli/sentinel_admin_signer/tests/test_ledger.py`
- [ ] T012 [P] `cli/sentinel_admin/sentinel_admin/auditlog.py`: writer JSONL append-only
      hash-encadenado **propio** (implementación independiente, no importa `ledger.py` del
      signer) para el audit-log local de `sentinel-admin` en el host del cliente —
      metadata-only, sin secretos ni payloads (data-model.md § Audit-log local) + tests en
      `cli/sentinel_admin/tests/test_auditlog.py`
- [ ] T013 [P] Gate offline en `cli/sentinel_admin_signer/tests/test_offline_gate.py`: check
      de imports prohibidos (requests/httpx/urllib.request/socket fuera de allowlist)
      sobre `sentinel_admin_signer/` + marker para correr la suite con red cortada
      (FR-011/SC-005)
- [ ] T014 [P] Gate offline en `cli/sentinel_admin/tests/test_offline_gate.py`: mismo check
      de imports prohibidos sobre `sentinel_admin/` + marker de red cortada (FR-011/SC-005)
...
**Artefacto**: exclusivamente `sentinel-admin-signer` (FR-017). Ningún task de esta fase
toca `cli/sentinel_admin/`.
...
**Independent Test**: quickstart pasos 1-3 — build de `sentinel-admin-signer`, contract test
...
- [ ] T015 [P] [US1] Tests primero en
      `cli/sentinel_admin_signer/tests/test_license_issue.py`: emisión por flags produce
...
- [ ] T016 [US1] `cli/sentinel_admin_signer/sentinel_admin_signer/signing.py`: emisor que
...
- [ ] T017 [US1] `cli/sentinel_admin_signer/sentinel_admin_signer/cmd_license.py`: comandos
      `issue` / `issue-dev` (defaults dev + advertencia `SENTINEL_ALLOW_DEV_LICENSE`) /
      `verify` según contrato (`sentinel-admin-signer license ...`); wiring a
...
- [ ] T018 [US1] `cmd_license.py`: `keyset export` (formato `# key_id:` del verifier,
      ...
      doble) + tests CliRunner en `cli/sentinel_admin_signer/tests/test_keyset.py`
- [ ] T019 [US1] Contract test del artefacto en
      `cli/sentinel_admin_signer/tests/test_contract_artifact.py` (FR-020, parte 1): el `.pyz`
      de `sentinel-admin-signer` buildeado emite un `.lic` → `docker compose run backend` lo
      valida con `verify_license_blob` (cubre el empaquetado, quickstart paso 1)

**Checkpoint**: US1 entregable sola — Sentinel ya puede firmar sin editar código, desde un
...
**Artefacto**: exclusivamente `sentinel-admin` (FR-018). Ningún task de esta fase importa ni
referencia código de `cli/sentinel_admin_signer/`.
...
fuera de banda, nunca ejecutando `sentinel-admin-signer` en este host), la secuencia completa
de `sentinel-admin` contra el stack dev termina con `install verify` en verde
...
### Comandos de `sentinel-admin` (host del cliente)

- [ ] T023 [P] [US2] `cli/sentinel_admin/sentinel_admin/profile.py` + comandos `install profile
      ...
      `cli/sentinel_admin/tests/test_profile.py`
- [ ] T024 [P] [US2] `cli/sentinel_admin/sentinel_admin/secrets.py` + `install secrets gen`:
      secretos del compose prod con `secrets`/`os.urandom` a archivo `600`; regenerar pide
      confirmación + tests
- [ ] T025 [US2] `cli/sentinel_admin/sentinel_admin/bundle.py` + `install bundle create/verify` +
      ...
      empaqueta el **layout completo de FR-021** — `bin/{sentinel-admin-x86_64.pyz,
      sentinel-admin-arm64.pyz}` (sha256 + versión de release en el MANIFEST, FR-022),
      ...
      índice externo, FR-023) — **nunca** empaqueta nada de `sentinel-admin-signer` + tests en
      `cli/sentinel_admin/tests/test_bundle.py`
- [ ] T026 [US2] `cli/sentinel_admin/sentinel_admin/stack.py` + `install up`: puebla los
...
- [ ] T027 [US2] `cli/sentinel_admin/sentinel_admin/cmd_license.py`: `install license verify`
...
- [ ] T028 [US2] `install seed apply` / `install admin bootstrap`: wrappers de
      `sentinel-admin` hacia los entrypoints T020/T021 vía `docker compose exec` con la UX del
...
- [ ] T030 [US2] Test de flujo completo en `cli/sentinel_admin/tests/test_install_flow.py`
      (CliRunner + stack dev): la secuencia entera del quickstart paso 4 en verde, usando
      únicamente `sentinel-admin` (el host de prueba no tiene `sentinel-admin-signer` instalado)

**Checkpoint**: MVP completo — firma (`sentinel-admin-signer`) + instalación guiada
(`sentinel-admin`), cada uno funcional de forma independiente
...
- [ ] T031 [P] Test de inspección de artefacto en
      `cli/sentinel_admin/tests/test_artifact_boundary.py` (FR-020, parte 2 — **release
      gate obligatorio**): abre el `.pyz` de `sentinel-admin` recién buildeado como zip
      ...
      la privada del emisor, o una dependencia declarada hacia `sentinel_admin_signer`;
...
- [ ] T032 [P] Test de contenido del bundle en
      `cli/sentinel_admin/tests/test_bundle_contents.py` (FR-021/FR-023 — **release gate
      ...
      los cinco top-level esperados por FR-021: `bin/{sentinel-admin-x86_64.pyz,
      sentinel-admin-arm64.pyz}`, `images/`, `manifests/`, `profiles/`, `install.sh` (falla si
      ...
      `sentinel-admin-signer*` en **cualquier** ubicación del tarball — cubre tanto el caso
...
- [ ] T033 Wiring del MANIFEST del bundle (FR-022): sha256 + versión de release de cada
      `sentinel-admin-<arch>.pyz` junto a imágenes/migraciones/perfiles/manifests — misma
      versión en todos; test de aserción del MANIFEST en
      `cli/sentinel_admin/tests/test_bundle_contents.py` (mismo archivo que T032, casos
...
- [ ] T034 Guarda de regresión anti-runtime-gating (FR-019): extiende
      `cli/sentinel_admin/tests/test_artifact_boundary.py` (T031) con un grep estático sobre
      el código fuente de `sentinel_admin/` y `sentinel_admin_signer/` que falla si aparece
      cualquier flag de rol (`--role`), variable de entorno de gate (`SENTINEL_ROLE`,
      `SENTINEL_SIGNER_MODE` o similar) o grupo Click oculto (`hidden=True`) — documenta el
...
- [ ] T035 [P] Whitelabel extendido: strings de `--help`/errores de **ambos** `.pyz`
      (`sentinel-admin-signer` y `sentinel-admin`, todas las arquitecturas) pasan
...
- [ ] T036 [P] Deprecar los scripts viejos con puntero al artefacto correspondiente
      (FR-012): `issue_dev_license.py` → banner apuntando a `sentinel-admin-signer license
      issue`; `apply_profile_seed.py`/`generate_trueup.py` → banner apuntando a
      `sentinel-admin install seed apply` / al comando `ops trueup export` (fase 2) — doc de
      migración en `backend/scripts/README.md`
- [ ] T037 [P] Docs DoD: página/runbook de **ambos** artefactos en la doc de producto
      (install-deploy actualizado para distinguir `sentinel-admin-signer` de `sentinel-admin`,
...
- **US2 (T020-T030)**: independiente tras Foundational; los entrypoints T020-T022 y los
  ...
  licencia dev, sin ejecutar `sentinel-admin-signer`
- **Empaquetado/frontera (T031-T035)**: requiere que US1 (T019, build del signer) y US2
  ...
  `sentinel-admin`, T032/T033 verifican el bundle de T025
...
Frente A (US1, cli/sentinel_admin_signer/): T015 → T016 → T017 → T018 → T019
Frente B (US2, cli/sentinel_admin/ + backend/):
...
**MVP first**: US1 sola ya entrega valor (Sentinel firma sin editar código, desde un
...
única garantía de que `sentinel-admin` no arrastra el signer, y debe estar en verde antes de
