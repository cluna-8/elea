# Feature Specification: CLI de operador (`sentinel-admin`) — firma de licencias e instalación guiada offline

**Feature Branch**: `026-cli-operador-sentinel`
...
- **`generate_trueup.py`** vuelca el JSON firmado a **stdout**: si falta el `>` se pierde; el envío a Sentinel es un paso humano fuera de banda sin tracking.
...
- **Q**: ¿La CLI se distribuye como un único binario universal (con comandos de firma y de
  ...
  **A**: **Dos artefactos físicamente separados**: `sentinel-admin-signer` (herramienta interna de
  firma de Sentinel: emisión de licencias, custodia de clave privada PKCS8 cifrada, import/unlock
  de clave, rotación de clave del emisor, export del keyset público, ledger de emisiones,
  verificación de licencias — corre solo en la estación de firma aislada de Sentinel, nunca se
  entrega a partners/clientes ni se incluye en el bundle air-gapped) y `sentinel-admin` (herramienta
  ...
  código de emisión, custodia de clave privada de Sentinel, rotación de clave del emisor, comandos
...
- **Q**: ¿Cómo se aplica y verifica esa separación? → **A**: En **build time**, con dos tests
  obligatorios: un contract test para `sentinel-admin-signer`, y un test de inspección de artefacto
  que falla el build si `sentinel-admin` contiene módulos, símbolos, comandos o dependencias
  exclusivos del signer.
- **Q**: ¿Qué va dentro del bundle air-gapped y con qué layout? → **A**: Únicamente
  `sentinel-admin` (nunca `sentinel-admin-signer`). Layout esperado:
  `bundle-v<version>.tar.gz` con `bin/{sentinel-admin-x86_64.pyz, sentinel-admin-arm64.pyz}`,
  `images/`, `manifests/`, `profiles/`, `install.sh`.
- **Q**: ¿Qué garantías deben cumplir los `.pyz` operacionales y cómo se instala `sentinel-admin`
  ...
  Python 3.11+ y Docker en el host destino. `sentinel-admin` NUNCA se instala vía pip, apt, yum ni
  repositorios externos dentro del entorno air-gapped — la única vía es el `.pyz` del bundle.
  Matriz de despliegue: estación de firma de Sentinel → `sentinel-admin-signer`; máquina de build/CI
  del partner → `sentinel-admin`; host air-gapped del cliente → `sentinel-admin` empaquetado en el
  bundle.
...
Un humano de Sentinel (con la clave privada de firma custodiada) necesita emitir un `.lic`
...
Un ingeniero (de Sentinel o del partner) instala un cliente nuevo: arma el perfil, genera
...
- **Licencia dev en deployment de producción** → respeta el guard existente
  (`SENTINEL_ALLOW_DEV_LICENSE`); no permite mezclar dev/prod en silencio.
...
- **Secretos/virtual keys** → siempre a archivo con permisos restringidos; nunca a stdout ni
  a logs.
- **Build de `sentinel-admin` que arrastra código/dependencias del signer** → el test de
  inspección de artefacto (build time) DEBE fallar el build antes de que pueda distribuirse.
- **`sentinel-admin-signer` incluido por error en el armado del bundle air-gapped** → nunca debe
  ocurrir; el proceso de empaquetado del bundle solo toma `sentinel-admin` como entrada.
...
**Núcleo — firma (US1, MVP) — exclusivo de `sentinel-admin-signer`**

- **FR-001**: `sentinel-admin-signer` DEBE emitir un `.lic` de producción firmado tomando el
...
- **FR-003**: La clave privada de firma NUNCA DEBE quedar escrita en claro en disco; se maneja
  **cifrada en reposo (PKCS8)** dentro de `sentinel-admin-signer` y se usa sin exponerla.
- **FR-004**: `sentinel-admin-signer` DEBE poder **exportar el keyset público** (kid + clave) como
  archivo para embeber/distribuir, sin exponer la privada.

**Núcleo — instalación (US2, MVP) — exclusivo de `sentinel-admin`**

- **FR-005**: `sentinel-admin` DEBE cubrir el camino de **cero a stack corriendo y verificado**:
...
- **FR-006**: Toda operación que mute datos productivos (seed que renombra tenant, instalación
  ...
  antes de aplicar.
- **FR-007**: Antes de mutar la base, `sentinel-admin` DEBE **validar a qué deployment/DB apunta**;
...
- **FR-010**: `sentinel-admin` DEBE ofrecer una **verificación end-to-end** post-deploy
...
- **FR-011**: Todos los comandos del núcleo (firma en `sentinel-admin-signer` + instalación y
  operación día-2 en `sentinel-admin`) DEBEN correr **offline, sin egress ni phone-home**,
...
- **FR-012**: `sentinel-admin-signer` y `sentinel-admin` DEBEN, entre los dos, **reemplazar** (no
...
- **FR-016**: La solución DEBE producirse como **dos artefactos físicamente separados**:
  `sentinel-admin-signer` (herramienta interna de firma de Sentinel) y `sentinel-admin` (herramienta de
  instalación/operación offline para partners/clientes). NO DEBE existir un binario único que
  combine ambos roles.
- **FR-017**: `sentinel-admin-signer` corre únicamente en la estación de firma aislada de Sentinel;
...
- **FR-018**: `sentinel-admin` puede contener los comandos de perfil, secretos, bundle, load, up,
  ...
  código de emisión de licencias, custodia de la clave privada de Sentinel, rotación de la clave
...
- **FR-020**: La separación entre `sentinel-admin-signer` y `sentinel-admin` DEBE aplicarse en **build
  time** y verificarse con tests: un contract test para `sentinel-admin-signer`, y un test de
  inspección de artefacto que **falle el build** si `sentinel-admin` contiene módulos, símbolos,
  comandos o dependencias exclusivos del signer.
- **FR-021**: El bundle air-gapped DEBE distribuir únicamente `sentinel-admin` (nunca
  `sentinel-admin-signer`), con el layout:

  ```
  bundle-v<version>.tar.gz
  ├── bin/
  │   ├── sentinel-admin-x86_64.pyz
  │   └── sentinel-admin-arm64.pyz
  ├── images/
  ├── manifests/
  ├── profiles/
  └── install.sh
...
- **FR-023**: `sentinel-admin` NO DEBE instalarse vía pip, apt, yum ni repositorios externos dentro
  del entorno air-gapped; la única vía de instalación es el `.pyz` incluido en el bundle.

### Key Entities *(include if feature involves data)*

- **`sentinel-admin-signer`** (artefacto separado, interno Sentinel): herramienta de la estación de
...
- **`sentinel-admin`** (artefacto separado, distribuido en el bundle): herramienta de
...
- **Licencia `.lic`**: artefacto firmado (definido en 021) — `sentinel-admin-signer` lo **produce**
  (US1) y `sentinel-admin` lo **instala** (US2). No redefine su formato.
- **Keyset público del emisor** (kid + clave): `sentinel-admin-signer` lo **exporta**; jamás lo pisa
  sin rotación explícita.
- **Clave privada de firma**: custodiada, cifrada en reposo (PKCS8), solo dentro de
  `sentinel-admin-signer` en la estación de firma de Sentinel — se usa para firmar sin exponerla; nunca
  existe en `sentinel-admin`.
- **Perfil de cliente** (config + branding + env): `sentinel-admin` lo **scaffolda y renderiza**
  desde el ejemplo, validando el layout y las variables del template.
- **Bundle air-gapped** (tarball de imágenes + MANIFEST + `sentinel-admin` `.pyz` por arquitectura):
  se **crea, verifica y carga** garantizando que ninguna imagen ni artefacto falte antes de la
  transferencia; nunca incluye `sentinel-admin-signer`.
- **Registro de emisiones / audit-log local**: traza offline de lo que cada artefacto hizo (qué
  se emitió por tenant vía `sentinel-admin-signer`, qué se instaló vía `sentinel-admin`), exportable
  como archivo.
...
- **Empaquetado — DECIDIDO (ver Clarifications 2026-08-05, FR-016 a FR-023)**: dos artefactos
  físicamente separados, `sentinel-admin-signer` (interno Sentinel, estación de firma aislada) y
  `sentinel-admin` (partner/cliente, distribuido en el bundle air-gapped). Un binario único
  ...
  artefacto sobre `sentinel-admin`.
- **Matriz de despliegue**: estación de firma de Sentinel → `sentinel-admin-signer`; máquina de
  build/CI del partner → `sentinel-admin` (se construye ahí); host air-gapped del cliente →
  `sentinel-admin` empaquetado dentro del bundle. `sentinel-admin-signer` nunca corre ni se distribuye
  fuera de la estación de firma de Sentinel.
