# Data Model — 026 CLI de operador (`basa-admin` + `basa-admin-signer`)
...
(ledger de emisiones — exclusivo de `basa-admin-signer` — y audit-log local — uno por
...
- **Invariante**: el emisor (`basa-admin-signer`) **importa** `canonical_payload_bytes` de
  ...
  `basa-admin` también puede importarla para `license verify` (research.md D6).
...
**Invariantes** (ambos exclusivos de `basa-admin-signer` — es el único que rota/exporta):
(a) rotar = **AGREGAR** un bloque nuevo, nunca reemplazar el archivo (el foot-gun
histórico); (b) exportar nunca incluye material privado. `basa-admin` solo **consume**
...
`basa-admin` valida el layout y las variables del template antes de renderizar.
...
digests **+ el `.pyz` de `basa-admin` por arquitectura** (FR-021/FR-022 — nunca el de
`basa-admin-signer`). **Invariante**: `basa-admin bundle verify` comprueba que TODAS las
...
(`BASA_LICENSE_TOKEN[_FILE]`), génesis de la cadena de audit. `basa-admin` **no** las
...
### Clave privada de firma cifrada (exclusiva de `basa-admin-signer`)
...
- **Qué**: la privada Ed25519 del emisor, **cifrada en reposo** como PEM PKCS8 con
  ...
  únicamente en la estación de firma de Basa; `basa-admin` no la genera, no la custodia y
...
### Ledger local de emisiones (exclusivo de `basa-admin-signer`)
...
- **Qué**: JSONL append-only por máquina: `{ts, comando, args_sanitizados, resultado,
  ...
  producto). `basa-admin-signer` escribe el suyo en la estación de firma; `basa-admin`
...
- **Keyset**: `sin claves` → `1 kid activo` → (rotación explícita) → `N kids` (todas
  ...
  con licencias vivas — `basa-admin-signer` lo bloquea salvo `--force` con confirmación
...
- **Instalación** (secuencia de US2): `perfil` → `secretos` → `bundle verificado` →
  ...
  → `verificado`. Cada paso valida sus precondiciones; `basa-admin` puede retomar desde el
