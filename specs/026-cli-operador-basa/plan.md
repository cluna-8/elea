# Implementation Plan: CLI de operador (`basa-admin` + `basa-admin-signer`) — firma de licencias e instalación guiada offline

**Branch**: `026-cli-operador-basa` | **Date**: 2026-07-22 | **Last revised**: 2026-08-05 (re-plan tras Clarifications) | **Spec**: [spec.md](spec.md)
...
- **`basa-admin-signer`** — herramienta interna de Basa. Corre únicamente en la estación
...
- **`basa-admin`** — herramienta de partner/cliente. Instalación guiada de un cliente
  ...
  custodia de la privada de Basa, ni comandos exclusivos de firma — ni como módulo
  transitivamente importable.

Decisión de lenguaje (JF): **Python** para ambos artefactos — el emisor (`basa-admin-signer`)
...
zipapps `.pyz` single-file independientes** — solo el `.pyz` de `basa-admin` (por
arquitectura x86_64/arm64) viaja dentro del bundle air-gapped; `basa-admin-signer` se
construye y se queda en la máquina de firma de Basa, jamás en el bundle. La separación se
aplica en **build time** (dos paquetes Python, dos targets de build) y se verifica con
tests: un contract test para `basa-admin-signer` y un test de inspección de artefacto que
falla el build si `basa-admin` arrastra módulos/símbolos del signer (FR-020). Patrón de
...
doc 025). `basa-admin-signer` corre en la estación de firma de Basa (mismo prerrequisito,
...
tocan la privada del emisor de Basa → elegibles para `basa-admin`. La custodia/firma con
la privada del emisor (`keycustody.py`, `signing.py`, ledger de emisiones) es código
**nuevo, exclusivo de `basa-admin-signer`**, nunca importado por `basa-admin`. Cero
...
O_EXCL) **solo dentro de `basa-admin-signer`**, ledger de emisiones (lado Basa,
`basa-admin-signer`) y audit-log local por máquina (ambos artefactos, cada uno el suyo)
...
para flujos de comandos, **contract test del artefacto `basa-admin-signer`** (firma → el
backend del stack valida: cubre el empaquetado, no solo el código — FR-020), **test de
inspección de artefacto de `basa-admin`** (build time: falla si el `.pyz` contiene
...
tarball no tiene la capa completa `bin/{basa-admin-x86_64.pyz, basa-admin-arm64.pyz}` +
`images/` + `manifests/` + `profiles/` + `install.sh`, o si aparece `basa-admin-signer*`
...
**Target Platform**: `basa-admin-<arch>.pyz` (zipapp via shiv) por plataforma linux
...
`bundle create` (FR-021) — el único mecanismo sancionado para colocar `basa-admin` en el
...
repositorios externos dentro del air-gap). `basa-admin-signer.pyz` se construye para la
arquitectura de la estación de firma de Basa (single-arch, no viaja en ningún bundle —
...
**Project Type**: dos paquetes Python nuevos en `cli/` (`basa_admin_signer/` y
`basa_admin/`) + endurecimiento de entrypoints de dominio en
`backend/src/cli_entrypoints/` (módulos `python -m` que reemplazan los scripts sueltos,
usados por `basa-admin` para lo que toca SQLAlchemy).
...
**Scale/Scope**: MVP se reparte en dos artefactos — `basa-admin-signer` (~5 comandos:
`license issue`/`issue-dev`/`keyset export`/`keyset rotate`/`license verify`) y
`basa-admin` (~11 comandos: `install profile new`/`render`, `secrets gen`,
...
|---|---|---|
| I. Privacy & Masking-First | ✅ n/a | No toca el pipeline de masking; `basa-admin install verify` lo ejercita como smoke |
...
| IV. Onboarding as Data | ✅ refuerza | `basa-admin install profile new`/`seed apply` materializan "cliente = config + seed, nunca código" con validación |
| V. Cost Governance | ✅ n/a | Sin cambios |
| VI. LiteLLM-Native, No Patching | ✅ | El motor no se toca; `basa-admin` orquesta compose y entrypoints del backend |
...
├── contracts/
│   └── cli-comandos.md       # Contrato por comando (basa-admin-signer + basa-admin) + invariantes globales
...
├── basa_admin_signer/                      # Artefacto 1 — SOLO estación de firma Basa (FR-016/FR-017)
│   ├── pyproject.toml                      # deps: click + cryptography; entry point basa-admin-signer
│   ├── basa_admin_signer/
│   │   ├── __main__.py                     # python -m basa_admin_signer
...
│   │                                        #   (implementación propia — sin import cruzado hacia basa_admin)
│   ├── tests/
...
│
├── basa_admin/                              # Artefacto 2 — partner/cliente (FR-018), distribuido en el bundle
│   ├── pyproject.toml                      # deps: click + cryptography; entry point basa-admin
│   │                                        # SIN dependencia de basa_admin_signer (verificado por test)
│   ├── basa_admin/
...
│   │   ├── ui.py                           # confirm/--yes, prompts sin eco, errores legibles, exit codes 0-4
│   │   │                                    #   (implementación propia — sin import cruzado hacia basa_admin_signer)
│   │   └── auditlog.py                     # audit-log local propio de basa-admin, JSONL hash-encadenado
...
│   │   └── test_bundle_contents.py         # FR-021/FR-023: bundle contiene EXACTAMENTE bin/{basa-admin-x86_64.pyz,
│   │                                        #   basa-admin-arm64.pyz} + images/ + manifests/ + profiles/ + install.sh;
│   │                                        #   escaneo del tarball COMPLETO (no solo bin/) para basa-admin-signer*
...
backend/
├── src/cli_entrypoints/              # NUEVOS módulos python -m endurecidos (dominio SQLAlchemy) — usados por basa-admin
...
│                                      #   verifier.py/token.py/deployment_key.py/trueup_export.py → basa-admin OK
│                                      #   (no tocan la privada del emisor); custodia/firma del emisor es código
│                                      #   nuevo exclusivo de basa_admin_signer/ (no vive acá)
└── scripts/                          # los scripts viejos quedan deprecados con puntero al artefacto correspondiente

deploy/
├── release/                          # bundle.sh/render_profile.sh: basa-admin absorbe su contrato, incluyendo
│                                      # la generación de install.sh dentro del tarball (FR-021)
│                                      # (el bundle SOLO empaqueta el .pyz de basa-admin — nunca basa-admin-signer)
└── Makefile                          # targets cli-build-signer/cli-build-admin/cli-check integrados al gate
```

**Structure Decision**: **dos paquetes Python autocontenidos** en `cli/` — `basa_admin_signer/`
y `basa_admin/` — sin dependencia cruzada entre ellos (verificado por test de inspección
de artefacto, FR-020). `basa_admin` importa de `backend/src/licensing/` únicamente los
módulos que no tocan la privada del emisor de Basa (research.md D6); la custodia/firma de
esa privada es código nuevo que vive exclusivamente en `basa_admin_signer/`. Entrypoints
de dominio endurecidos en el backend existente (patrón D4), usados solo por `basa_admin`.
...
seguridad válida entre firma Basa e instalación partner/cliente — ningún gate en runtime
