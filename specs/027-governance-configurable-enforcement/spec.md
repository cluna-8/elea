...
Además, **qué capas tienen sentido depende del modo de conexión**: cuando el tráfico viaja contra la suscripción del propio cliente, el proveedor upstream ya aplica sus propias defensas; cuando el tráfico va a un modelo administrado por la pasarela (incluido un modelo local), **no hay nadie más protegiendo**. Hoy el producto no distingue esos dos mundos.
...
Un administrador decide qué capas opcionales aplican para el tráfico de **suscripción** y cuáles para el tráfico hacia **modelos administrados por la pasarela**, y puede afinarlo por **superficie/herramienta** (herramientas de código, navegador, API, interfaz propia). El **piso no-negociable** está siempre activo y no puede apagarse.
...
**Independent Test**: Configurar capas distintas para cada modo, enviar tráfico por ambos, y verificar que las capas aplicadas difieren según lo configurado y que el piso se aplica siempre en los dos casos.

**Acceptance Scenarios**:

1. **Given** una capa opcional habilitada solo para modelos administrados por la pasarela, **When** llega tráfico de suscripción, **Then** esa capa no se aplica y el registro del pedido lo refleja.
2. **Given** la misma capa, **When** llega tráfico hacia un modelo administrado por la pasarela, **Then** la capa se aplica y queda registrada como aplicada.
...
- **FR-010**: El sistema MUST ofrecer al administrador un resumen por modo de conexión que responda "qué protege hoy al tráfico de suscripción y qué protege al tráfico hacia modelos de la pasarela".
...
- **Modo de conexión**: distingue el tráfico que va contra la suscripción del propio cliente del tráfico que va hacia modelos administrados por la pasarela.
- **Superficie/herramienta**: el tipo de cliente que origina el tráfico (herramienta de código, navegador, API, interfaz propia).
