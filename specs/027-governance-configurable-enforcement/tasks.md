...
> `sentinel_guardrail.py`, `custom_auth.py`, `guardians.py`): no arrancarlas hasta rebasear sobre
...
[#40](https://github.com/DrZuzzjen/sentinel-guardian/issues/40) identidad del motor en prod ·
[#41](https://github.com/DrZuzzjen/sentinel-guardian/issues/41) e2e flaky (**cerrado en esta rama**: causa raíz era una carrera del teardown con el logger del motor) ·
[#42](https://github.com/DrZuzzjen/sentinel-guardian/issues/42) **la cobertura de patrones de secretos es de 2023 y 4 formatos vigentes pasan sin bloquear** — capa de piso, prioridad alta.
...
- [x] T007 **Resolutor puro** `resolve_profile` + `build_attribution` en `litellm/extensions/sentinel_governance.py` (contrato resolutor-perfil completo: firma tri-estado con `surface_trusted` y `connection_overrides`, precedencia Connection > superficie confiable > modo > tenant > producto, piso irrepresentable como apagado, relajar-exige-confiable, atribución con `applied_layers`/`blocked_by_layer`). `apply_layers` —la variante que EJECUTA capas, contrato #8— llega en US2/T025-T028. Unit tests exhaustivos + hostiles (perfil inyectado, veredictos basura).
...
**Independent Test**: quickstart SC-001/SC-002 — las 3+1 capas reales `aplicandose`, las de proveedor `no_disponible`/`requiere_credencial`, resumen por modo en una vista; `docker stop sentinel-litellm` → `no_disponible`, jamás activa.
...
- [ ] T019 🔀 [US1] Evidencia por pedido: `sentinel_guardrail.py` adopta `add_guardrail_to_applied_guardrails_header`; `chat.py`/`gateway.py` leen `x-litellm-applied-guardrails` como fuente C del estado.
...
- [x] T022 [P] [US2] Integration por alcance en `backend/tests/integration/test_governance_enforcement.py`: capa on solo para `gateway-models` → suscripción la lleva `delegated`/`skipped` jamás `applied` (exhaustividad); surface off confiable aplica, UA-derivada no; `X-Sentinel-Redact` fuerza ON pero jamás OFF; tri-estado (Connection sin toggle NO tapa la cascada); redact-off ⇒ `pii_detection: applied+count` + `pii_masking: skipped`.
...
- [ ] T024 🔀 [US2] `custom_auth.py`: perfil resuelto del tenant en `metadata['sentinel']` (vía SQL de identidad + cache), **fin del colapso NULL→True** (:149 — tri-estado crudo, contrato resolutor #2), mecanismo de invalidación por key (60 s → invalidable), y regla explícita para la master key (piso + perfil default, riesgo del research).
- [ ] T025 🔀 [US2] `sentinel_guardrail.py`: el hook consume el `Profile` (reemplaza el bloque fijo :82-99), escribe `home['sentinel_governance']` con `applied_layers`/`blocked_by_layer`, y **emite el evento de monitor en el punto de bloqueo** (contrato evento §12, best-effort, preview display-masked §10).
...
- [x] T029 [US2] `sentinel_audit_logger.py`: `applied_layers`/`blocked_by_layer` en `_INSERT_AUDIT_SQL` y en el evento del monitor (mismo esquema que gateway — contrato §8).
...
- [x] T037 `speckit-analyze` CORRIDO (cobertura 100% sobre 22 requisitos / 37 tareas, 0 duplicados, 1 CRITICAL + 2 HIGH). **Sync hecho**: terminología "pasarela"→"Modelo propio" en spec/quickstart/contracts/research (token de wire `gateway-models` intacto), SC-002/FR-010 realineados a la frase que rinde la UI, taxonomía de superficies acotada al enum declarado (navegador/API resuelven por modo — issue #28), `data-model.md` sincronizado con el RLS real de la migración 012, `plan.md` apuntando a `sentinel_governance.py`. **CRITICAL F1 CERRADO (2026-08-04, OK explícito de JF)**: enmienda D8 sellada — constitución **2.0.0→2.1.0** con Sync Impact Report; el Principio I ahora parte el mandato en piso no-negociable (detectar/evaluar/registrar) y transformar gobernable (capa `pii_masking`, jamás silencioso).
