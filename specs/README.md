...
| [056 Ingreso con Microsoft Entra ID (SSO) en Eleia Hub](056-sso-entra-id-hub/) | 📝 **Draft** (23-sep) | Pedido del dueño: activar Entra ID para Elea (usan Active Directory) sin perder funcionalidad. El SSO Entra **ya existe en la base** (spec 017, `backend/src/sso/`) pero está apagado en Elea (licencia sin `sso`, instalador sin `SENTINEL_SSO_REDIRECT_URI`) y **el Hub no tiene SSO**. Fase 1 = SSO en el Hub, activado desde el panel en el server de Elea, 5-7,5 días, desplegable apagado y con vuelta atrás ([DESPLIEGUE-Y-REVERSION](056-sso-entra-id-hub/DESPLIEGUE-Y-REVERSION.md), [SOLICITUD-A-ELEA](056-sso-entra-id-hub/SOLICITUD-A-ELEA.md)). Supuestos a confirmar con Elea: AD sincronizado con Entra, UPN = email, HTTPS/DNS. Preparada para portar a Sentinel. Sin `plan.md` ni `tasks.md`. |
