# Feature Specification: Basa Secure AI Gateway (by basa dev)

**Feature Branch**: `001-basa-ai-gateway`
...
**Input**: User description: "Un producto para el espacio de datos sanitario by basa dev, que actúe como una pasarela segura (gateway) con usuarios, presupuestos, seguridad (enmascaramiento PII/PHI con Presidio u otros), cumplimiento (GDPR y AI Act), y selector de LLM. Debe contar con una interfaz gráfica (UI) de 5 páginas (Usuarios/Presupuestos, Opciones de Seguridad, Políticas/Plantillas, Auditoría y Playground con una animación de capas) y una API. Marca blanca sin menciones a litellm. Se agrega soporte para un optimizador de contexto opcional usando Headroom."
...
* **Virtual Keys**: Generated with a secure prefix `basa_sk_...`. Only the SHA-256 hash is stored in the database. The plain-text key is shown only once upon creation.
* **Key-level Governance**: Virtual keys enforce budgets, log transactions in `audit_logs` (storing `api_key_id`), and authorize API requests via `Authorization: Bearer basa_sk_...`.
...
* Any reference to `litellm` or `LiteLLM` in error responses is replaced with `Basa Gateway`.
...
- **Branding Customizability**: All UI components are custom-built, allowing complete white-labeling under the "Basa Secure AI Gateway (by basa dev)" brand.
