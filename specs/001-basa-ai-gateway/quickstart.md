# Quickstart & Validation Guide: Basa Secure AI Gateway (by basa dev)

This document provides step-by-step instructions to launch the Basa Secure AI Gateway in a local development environment and validate its core functionalities.
...
DATABASE_URL=postgresql://basa_admin:basasecurepass123@db:5432/basa_gateway
...
You should see:
- `basa-frontend` (running on port `80` or `3000`)
- `basa-backend` (running on port `8000`)
- `basa-litellm` (running on port `4000`, internal only)
- `basa-db` (running on port `5432`)
