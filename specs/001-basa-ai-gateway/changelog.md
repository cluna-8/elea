# Changelog: Basa Secure AI Gateway

**Proyecto**: Basa Secure AI Gateway (by basa dev)
**Rama**: `001-basa-ai-gateway`
...
- Se orquestaron **6 contenedores**: `basa-frontend`, `basa-backend`, `basa-litellm`, `basa-db` (los contenedores de Presidio se eliminaron por consumo excesivo de RAM).
...
| `APIKey` | `api_keys` | Llaves `basa_sk_...`, solo hash SHA-256 almacenado |
...
**Llaves virtuales**: generadas con `secrets.token_urlsafe(32)`, prefijo `basa_sk_`, solo hash SHA-256 en BD, clave en claro mostrada **una sola vez**.
