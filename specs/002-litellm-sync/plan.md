...
| IV. Containerized & White-Label | ✅ | LiteLLM sigue siendo interno. Los clientes solo ven nuestro backend. |
...
backend/src/
├── services/
│   └── litellm_client.py        ← NUEVO: wrapper HTTP para la API de LiteLLM
├── models/
│   ├── user.py                  ← MODIFICAR: agregar litellm_user_id, litellm_team_id
...
└── schemas/
    └── user.py                  ← MODIFICAR: agregar litellm_team_id en responses
...
## Architectural Decision: LiteLLMClient como servicio de infraestructura
...
### Fase 1 — LiteLLMClient + config (Foundational, sin UI)
...
Al crear un grupo, también se crea en LiteLLM. Se guarda `litellm_team_id`. Los grupos existentes sin `litellm_team_id` son legacy — no se migran.
...
Al crear un usuario, también se crea en LiteLLM. Se guarda `litellm_user_id`.
