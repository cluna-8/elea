...
- 🔨 **Rol Auditor (honesto)** — el panel ya gatea por rol (admin, developer, compliance_officer, usuario final), pero la ficha del auditor promete una red que no cubre costos/consumo/reportes; la matriz canónica lo hace verdad: VE todo lo de compliance, no toca nada. Rama quickwin arrancada (3 commits). *(034 **absorbida por la 017** el 13-ago — Capítulo A + `contracts/matriz-roles.md`, gate de producto sellado en PR #194; ver ROADMAP-guardian)*
- 🔨 **Rol Lectura** — solo-lectura general del panel; **especificado en la 017** (Capítulo A: cero mutaciones, no chatea, no prueba dueño, no consume seat); se implementa con la 017.
