...
**Departamento**: Guardian App Ecosystem (JF) · **Fecha**: 2026-08-15 · **Estado**: vigente — actualizado con los resultados del **examen 125** (gate PASS), el re-corte de ciclos al 5-SEP sellado por JF y los **2 nodos nuevos sellados el 12-ago** (región de compliance · compresión 023) · **STATUS 25-ago**: el nodo **SSO flipea a 🟢** con el E2E real ejecutado y verde (ver el nodo); región sigue 🔨 (falta la capa wizard); el denominador sigue en 26 nodos + 3 gates → **13/26 nodos (50%)**
...
- ✅ **Retención que borra de verdad** — la purga real existe: al día 91 el contenido clasificado `prompt_content` se borra solo (job + tiers estricto/estándar + scheduler APAGADO por default vía `SENTINEL_PURGE_ENABLED`), con rastro auditable metadata-only. *(spec 018 · PR #214, `61089b1`)*
- ⚖️ **¿Qué hace el firewall si NO puede auditar?** — hoy el default es fail-open (`SENTINEL_AUDIT_FAIL`). **Dirección acordada en el weekly 05-ago: política por nivel de riesgo/rol del usuario** (alta prioridad corta, baja prioridad sirve) — falta especificarla (spec corta, encaja con las capas de la 027). Pendiente conexo: ¿se auditan también los errores?
...
- ✅ **Rol Auditor (honesto)** — hecho vía **017 US1** (la 034 fue absorbida por la 017, decisión JF 13-ago; gate de producto sellado en PR #194): `compliance_officer` VE todo — IAM, config de producto, compliance, artefactos y vitrinas en lectura — y su única escritura es resolver human-reviews; la matriz-ley vive en `backend/src/auth/matrix.py` (contrato en `contracts/matriz-roles.md`) y el harness FR-005 (`tests/integration/test_role_matrix.py`, 15 routers) la fija en CI; frontend reconciliado a la matriz (#244/#250). **Verificado por QA persona-a-persona el 24-ago** (curl + JWT real): 14 lecturas → 200, 13 escrituras → 403, resolver pasa el gate, chat permitido — 30/30. Salvedad UX abierta: afordancias de escritura visibles que la API gatea igual (#261, no es seguridad). *(spec 017 US1 · PRs #246/#253/#254/#256)*
- ✅ **Rol Lectura** — hecho vía **017 US1**: rol `lectura` = solo-vitrinas (audit/reportes/costos en lectura, todo lo demás 403) y **chat interno negado fail-closed** (gate T010 como dependency, 403 con mensaje propio). QA persona Lectura verde el 21-ago; el 403 de chat re-confirmado el 24-ago en la pasada del auditor. *(spec 017 US1 · PRs #254/#256)*
- 🟢 **Login con Microsoft y Google (SSO escalón 1)** — verde por el criterio sellado en este nodo (Entra E2E-real completo; Google sigue en el escalonado) — para clientes que viven en Microsoft 365 / Google Workspace: entrar con la cuenta corporativa (OIDC). **Microsoft Entra está IMPLEMENTADO en `main` — ya no es vitrina mock**: proveedor OIDC authorization-code con discovery y allowlist de algoritmo (`JsonWebToken(["RS256"])`, `backend/src/sso/entra.py`), rutas `/auth/sso/login`, `/auth/sso/callback` y `/auth/sso/available` con el gate de licencia `feature_enabled('sso')` cableado como **dependency del router** (`api/__init__.py:48`, no inline: el 403 sale antes de tocar el body), alta JIT por email que jamás re-asigna rol, y el botón «Entrar con Microsoft» condicionado al flag. **Google NO está implementado**: el registry tiene un solo `register(...)` y es el de Entra; en la pantalla de SSO, Google/Okta/Auth0/Keycloak/SAML siguen con badge «Próximamente» — la vitrina que queda es honesta. **E2E contra el tenant Entra REAL: EJECUTADO Y VERDE el 25-ago** (cierra SC-003 y la mitad restante de T018): las 4 fases automáticas del harness (`frontend/e2e/017-sso-e2e.mjs`) dieron 30/30 pasos verdes sobre `main`, y la fase 5 (login-real) se ejecutó manualmente contra `login.microsoftonline.com` del tenant real — canje code→token con el secret sembrado, alta JIT con rol mínimo `client` verificada en DB (centinela anti-login-local en la fila), sesión viva en el portal de usuario. Evidencia con captura por paso: `RESEARCH/e2e-sso-017-25ago/` (workspace del manager) + reporte PDF del 25-ago. La batería sintética de `main` (`test_sso_entra.py` · `test_sso_api.py` · `test_sso_jit.py`) sigue cubriendo protocolo e invariantes en cada CI. **Escalonado**: Entra + Google ahora; escalón 2 (post-Fase 0): SAML, otros IdP y **Active Directory + segmentación por roles/grupos** — consulta crítica de cliente identificada en el weekly. *(017 US2 · T014 [#260] · T015/T016 [#266] · T017 [#263] · doc de instalación vendible [#270], mitad DOC de T018 ✅)*
...
- 🔨 **Reinicio de contenedores desde la UI** — botón «Aplicar cambios del motor» especificado junto a la 033 (alcance = D1, al sello de JF); alivio inmediato del dolor que la 033 elimina de raíz. *(spec 033 · #70)*
- 🔨 **Región de compliance elegida por el partner** — la región vive en el **TENANT** (canon de almacenamiento) y el wizard es la capa UX que la escribe al instalar; se activan solo los recognizers de esa región para que el NLP tenga menos ruido. **El backend por-tenant está en `main`**: `resolve_region()` con el mismo patrón que `resolve_nlp_fail_mode` (#63/#104), cableado en los cuatro planos —policy, firewall/gateway, motor (`sentinel_guardrail.py` + los dos `_IDENTITY_SQL` espejados de `custom_auth.py` e `internal.py`) y el seed de instalaciones nuevas—, con degrade explícito ante región no reconocida y auditoría del cambio (`region_previo`/`region_actual`); 9 tests lo fijan. **Lo que falta para 🟢 es la capa wizard que la escribe al instalar**: hoy no hay superficie de wizard en `backend/src` ni en `frontend/src`, y `specs/037-wizard-onboarding/` todavía no está en `main` — viene en el PR de shaping #206, con ownership de Cristian. **Nodo nuevo sellado por JF el 12-ago.** *(#174 + #137 · PRs #267/#268, autoría de Cristian preservada)*
...
Qué ES cada módulo (stack, soporte, frontera con Factory) está en `tech-team/EcosystemOverview-SentinelGuardian.html`; este árbol dice dónde está SU trabajo: Gateway (✅ · F0 streams/perillas) · Motor (F0 033 · ✅ fallback) · PII (F0 #63/#64) · Extensión (F1 multi-browser) · 3rd-party (F1 matriz) · Panel (✅ 11 páginas con roles · F0 huecos wizard) · Playground/monitor (✅) · Auto-router (✅ existe · F1 garantía) · Auditoría (F0: DSAR/018/⚖️ fail-open) · Presupuestos (✅ · F0 #73) · Gobernanza (✅ · F0 colas 027) · Multi-tenant (✅) · Licensing runtime (✅; emisión=Factory) · Docs contenido (✅; build=Factory).
...
**Camino crítico al 5-SEP** (lectura del 12-ago): la capacidad dejó de ser el riesgo (medida y sobrada) — el riesgo es que **identidad y ciclo de vida del dato (SSO, roles, 018, DSAR) tienen 0 líneas escritas** y deben caber enteros en C2. Por eso las specs se escriben YA, en el colchón de C1. — **Actualización medida del 24-ago**: los cuatro dejaron de tener 0 líneas y están en `main` (SSO `backend/src/sso/` · roles `auth/matrix.py` + harness FR-005 · purga 018 #214 · DSAR #217). El riesgo se corrió: ya no es «escribirlo», es **cerrar lo que sólo se probó contra sustitutos** — el E2E de SSO contra tenant real y la capa wizard de la región.
...
1. ~~**Licencia de test de 500 seats** para el harness~~ — **RESUELTO (11-ago)**: keypair descartable generado al provisionar el SUT + licencia efímera propia (muere con la infra); la privada `sentinel-dev-2026b` queda SOLO para installs de cliente. Verificado en el examen 125 (fingerprint firmado con licencia real de test de 130 seats).
