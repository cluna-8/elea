...
**Independent Test**: Generar el paquete para un partner de prueba desde su marca y su URL, verificar que el paquete resultante es instalable en el navegador y apunta al gateway correcto, y confirmar que aparece listado en el manifiesto del entregable con su huella (checksum).

**Acceptance Scenarios**:

1. **Given** la marca de un partner y la URL de su gateway, **When** el operador genera el paquete de la extensión, **Then** obtiene un paquete instalable cuyo nombre, descripción, íconos y sitios interceptados salen de la marca del partner, sin ninguna edición manual.
2. **Given** una URL de gateway remota que no es segura (no `https`) y no es un entorno local, **When** el operador genera el paquete, **Then** la generación **falla con un error** (no una advertencia) — una credencial por usuario sobre una conexión insegura viaja en claro.
3. **Given** un paquete generado para un partner, **When** se arma el entregable de instalación, **Then** el paquete queda incluido en el entregable y listado en su manifiesto con su checksum.
4. **Given** dos empleados que instalan el mismo paquete del partner en carpetas distintas, **When** ambos lo cargan en el navegador, **Then** ambas instalaciones tienen la misma identidad estable (mismo identificador de extensión).

---

### User Story 2 - El paquete lleva la marca del partner y ninguna otra (Priority: P1) · MUST-piloto
...
### User Story 3 - La extensión declara su nivel real de protección (Priority: P1) · MUST-piloto
...
### User Story 4 - La extensión explica el motivo real de un bloqueo de gobernanza (Priority: P1) · MUST-piloto
...
### User Story 5 - La sesión distingue "sin red" de "sin permiso" y se revalida sola (Priority: P1) · MUST-piloto
...
### User Story 6 - Gate de release que ve la extensión (Priority: P2) · SHOULD
...
### User Story 7 - El backend rechaza keys vencidas y de otra superficie (Priority: P2) · SHOULD
...
**Generación y entrega por partner (US1, US2)**

- **FR-001**: El sistema MUST generar el paquete instalable de la extensión de un partner a partir de su marca y la URL de su gateway, sin edición manual de archivos.
- **FR-002**: El paquete generado MUST tomar de la marca del partner: el nombre visible, la descripción, los íconos y los sitios que intercepta.
- **FR-003**: La generación MUST fallar (no advertir) cuando la URL del gateway es remota y no es segura (`https`), permitiéndola solo para entornos locales.
- **FR-004**: El paquete generado MUST tener una identidad estable e independiente de la carpeta donde se instale.
...
- **FR-008**: La dirección del gateway MUST venir horneada en el paquete del partner y NOT ser editable por el usuario final.

**Honestidad de protección (US3)**

- **FR-009**: El sistema MUST exponer, para la superficie de navegador, una declaración de nivel de protección que indique detección "por patrones" y un texto de alcance en lenguaje llano.
- **FR-010**: La interfaz de la extensión MUST mostrar ese nivel como un indicador ámbar ("cobertura parcial"), no verde, y sin la palabra "protegido".
- **FR-011**: Cuando el servidor no informe el nivel de protección, la extensión MUST asumir el nivel más conservador ("patrones").
- **FR-012**: El texto de alcance MUST provenir del servidor (fuente única de copy), NOT estar hardcodeado en la extensión, de modo que el día que la superficie use análisis lingüístico el mensaje cambie sin re-empaquetar.
- **FR-013**: El vocabulario del nivel de detección expuesto al usuario MUST NOT contener nombres de motor ni de tecnología.

**Bloqueos de gobernanza (US4)**

- **FR-014**: Ante una respuesta de bloqueo por política, la extensión MUST frenar el envío y mostrar el motivo provisto por el servidor.
- **FR-015**: Ante una respuesta de fallo sin indicación de bloqueo por política, la extensión MUST mostrar "servicio no disponible".
- **FR-016**: La extensión MUST NOT exponer identificadores internos crudos de la capa que bloqueó.

**Sesión y revocación (US5)**

- **FR-017**: La extensión MUST distinguir tres estados: conectado, no verificado (fallo de red, key conservada) y desconectado (permiso denegado, key borrada).
- **FR-018**: La extensión MUST tratar una revocación de plaza como estado distinto de una key inválida, con un mensaje propio, y borrar la key en ambos casos.
- **FR-019**: Un corte de red MUST NOT borrar la key.
- **FR-020**: La extensión MUST revalidar la sesión periódicamente (del orden de media hora) y al reabrir el navegador.
- **FR-021**: El estado de sesión MUST tener un único componente responsable.

**Endurecimiento del servidor (US6, US7)**

- **FR-022**: El proceso de release MUST fallar cuando un paquete generado contiene la marca del fabricante o nombres de internals (además de los nombres de motor ya prohibidos).
- **FR-023**: El gateway MUST rechazar una key vencida con el mismo error indistinguible que una key inexistente.
- **FR-024**: El gateway MUST rechazar, en la superficie de navegador, una key cuya herramienta no sea de superficie navegador.

### Key Entities *(include if feature involves data)*

- **Paquete de extensión del partner**: el artefacto instalable generado para un partner; deriva su identidad visible (nombre, descripción, íconos, sitios interceptados, URL de gateway, identificador estable) de la marca del partner.
...
- **SC-001**: Un operador genera el paquete de un partner desde su marca y su URL en un solo comando, sin editar ningún archivo del paquete a mano.
...
- **Decisiones de owner ya cerradas (no se re-litigan en esta spec)**: API key por usuario (el admin la emite); los partners rebrandean (no hay una ficha neutra única); sin gestión centralizada de dispositivos (instalación manual o zip a IT); la key y la URL son un setup de una vez que no se vuelve a mostrar ("es un login"); la protección PII no es desactivable por el usuario; la superficie de navegador detecta por patrones, no por análisis lingüístico (aceptado en PR #21).
...
- **Dependencia de la 027**: el motivo real de bloqueo (US4) se apoya en el contrato de bloqueo de la gobernanza (027), ya disponible en la rama de integración de trabajo; no requiere esperar a un merge externo.
